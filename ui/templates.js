angular.module('kityminderEditor').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui/directive/fileBtn/fileBtn.html',
    "<div class=\"km-btn-group dropdown file-btn\" is-open=\"isopen\" dropdown><div class=\"km-btn-primary dropdown-toggle\" dropdown-toggle ng-class=\"{'active': isopen}\"><i class=\"icon-menu icon\"></i> <span>{{ 'file' | lang:'ui' }}</span></div><ul class=\"dropdown-menu\"><li ng-click=\"new()\"><a href=\"javascript:;\">{{ 'new' | lang:'ui/file_dropdown' }}</a></li><li ng-click=\"openInFile()\"><a href=\"javascript:;\">{{ 'openInFile' | lang:'ui/file_dropdown' }}</a></li><li><a for=\"upload-km\" href=\"javascript:;\">{{ 'uploadLocal' | lang:'ui/file_dropdown' }} <input type=\"file\" id=\"upload-km\" name=\"upload-km\" class=\"upload-km\" accept=\".km,.mm,.xmind\" onchange=\"angular.element(this).scope().uploadImage()\"></a></li><li class=\"divider\"></li><li ng-click=\"save()\"><a href=\"javascript:;\">{{ 'save' | lang:'ui/file_dropdown' }}</a></li><li ng-click=\"rename()\"><a href=\"javascript:;\">{{'rename' | lang:'ui/file_dropdown'}}</a></li><li ng-click=\"saveAs()\"><a href=\"javascript:;\">{{ 'saveAs' | lang:'ui/file_dropdown' }}</a></li><li class=\"divider\"></li><li ng-click=\"generatePlan()\"><a href=\"javascript:;\">{{ 'generatePlan' | lang:'ui/file_dropdown' }}</a></li><li ng-click=\"export()\"><a href=\"javascript:;\">{{'exportMind' | lang:'ui/file_dropdown'}}</a></li></ul></div>"
  );
  ...
  $templateCache.put('ui/directive/fullScreen/fullScreen.html',
    "<div class=\"km-btn-group\"><div class=\"km-btn-item full-screen\" tooltip-placement=\"bottom\" tooltip-append-to-body=\"true\" tooltip=\"{{ getTip() }}\" ng-click=\"toggleIcon()\"><i ng-class=\"{'icons2-full_screen_out': out, 'icons2-full_screen_in': in}\"></i></div></div>"
  );
  ...
  $templateCache.put('ui/directive/kityminderEditor/kityminderEditor.html',
    "<div class=\"minder-editor-container\"><div class=\"toolbar\" ng-class=\"{'additional-padding': toolbarAdditionalPadding}\" toolbar=\"minder\" editor=\"editor\" state=\"state\" ng-if=\"minder\"></div><div search-box minder=\"minder\" ng-if=\"minder\"></div><div class=\"minder-editor\"></div><div note-editor minder=\"minder\" ng-if=\"minder\"></div><div class=\"note-previewer\" note-previewer ng-if=\"minder\"></div><div class=\"navigator\" navigator minder=\"minder\" ng-if=\"minder && !isMobile\"></div></div>"
  );
  ...
  $templateCache.put('ui/dialog/hyperlink/hyperlink.tpl.html',
    "<div class=\"hyper-link-dialog r-modal-shadow\"><div class=\"r-modal-header\"><h3 class=\"r-modal-title\">链接</h3><r-modal-close></r-modal-close></div><div class=\"r-modal-body\"><form><div class=\"form-group\" id=\"link-url-wrap\"><span for=\"link-url\">链接地址：</span> <a href=\"javascript:;\" ng-click=\"deleteCurrent()\" class=\"pull-right remove-link\">移除已有链接</a> <input type=\"text\" ng-model=\"url\" ng-blur=\"urlPassed = R_URL.test(url)\" ng-focus=\"this.value = url\" ng-keydown=\"shortCut($event)\" class=\"r-input\" placeholder=\"以http(s):// 或 ftp:// 开头\" id=\"link-url\"></div><div class=\"form-group\"><span for=\"link-title\">提示文本：</span> <input type=\"text\" ng-model=\"title\" ng-blur=\"titlePassed = true\" class=\"r-input\" id=\"link-title\"></div></form></div><div class=\"r-modal-footer\"><button class=\"btn r-btn-primary btn-block btn-xxl\" ng-click=\"ok()\">确定</button></div></div>"
  );
  ...
  $templateCache.put('ui/dialog/image/image.tpl.html',
    "<div class=\"image-dialog r-modal-shadow\"><div class=\"r-modal-header\"><h3 class=\"r-modal-title\">图片</h3><r-modal-close></r-modal-close></div><div class=\"r-modal-body\"><tabset><tab heading=\"图片搜索\"><form class=\"form-inline key-word-form\"><span for=\"search-keyword\" class=\"search-tip\">关键词：</span> <input type=\"text\" class=\"r-input\" ng-model=\"data.searchKeyword2\" id=\"search-keyword\" placeholder=\"请输入搜索的关键词\"> <button class=\"btn r-btn-primary search-btn\" ng-click=\"searchImage()\">百度一下</button></form><div class=\"search-result\" id=\"search-result\"><ul><li ng-repeat=\"image in list\" id=\"{{ 'img-item' + $index }}\" ng-class=\"{'selected' : isSelected}\" ng-click=\"selectImage($event)\"><img id=\"{{ 'img-' + $index }}\" ng-src=\"{{ image.src || '' }}\" alt=\"{{ image.title }}\" onerror=\"this.parentNode.removeChild(this)\"> <span>{{ image.title }}</span></li></ul></div><a href=\"javascript:;\" ng-click=\"deleteCurrent()\" class=\"remove-image\">移除已有图片</a></tab><tab heading=\"外链图片\"><form><div class=\"form-group\" ng-class=\"{true: 'has-success', false: 'has-error'}[urlPassed]\"><span for=\"image-url\">链接地址：</span> <input type=\"text\" class=\"r-input\" ng-model=\"data.url\" ng-blur=\"urlPassed = data.R_URL.test(data.url)\" ng-focus=\"this.value = data.url\" ng-keydown=\"shortCut($event)\" placeholder=\"以 http(s):// 开头\" id=\"image-url\"></div><div class=\"form-group\"><img class=\"image-preview\" id=\"image-preview\" ng-src=\"{{ data.url }}\" alt=\"{{ data.title }}\"></div></form><a href=\"javascript:;\" ng-click=\"deleteCurrent()\" class=\"remove-image\">移除已有图片</a></tab><tab heading=\"上传图片\" active=\"true\"><form><div class=\"form-group\"><input type=\"file\" name=\"upload-image\" id=\"upload-image\" class=\"upload-image\" accept=\".jpg,.JPG,jpeg,JPEG,.png,.PNG,.gif,.GIF\" onchange=\"angular.element(this).scope().uploadImage()\"><label for=\"upload-image\" class=\"btn r-btn-primary\">上传文件</label></div><div class=\"form-group\"><img class=\"image-preview\" id=\"image-preview\" ng-src=\"{{ data.url }}\" title=\"{{ data.title }}\" alt=\"{{ data.title }}\"></div></form><a href=\"javascript:;\" ng-click=\"deleteCurrent()\" class=\"remove-image\">移除已有图片</a></tab></tabset></div><div class=\"r-modal-footer\"><button class=\"btn r-btn-primary btn-block btn-xxl\" ng-click=\"ok()\">确定</button></div></div>"
  );
